...
if [ ! -r mcu.key ]
then
    # ECDSA P-256 (prime256v1) : standard WebRTC/DTLS, plus rapide que RSA
    openssl ecparam -name prime256v1 -genkey -noout -out mcu.key
    rm -f mcu.crt
else
...
then
    ...
    openssl x509 -req -sha256 -days 3650 -in mcu.csr -signkey mcu.key -out mcu.crt
