...
Les suites `*_hardening.cpp` (chantier 5, `network_parsers_hardening_plan.md`)
posent un problème que le round-trip ne résout pas : un parseur qui lit dix
...
| veto ICE rétabli sur nos SEULS credentials (`if (iceRemotePwd \|\| iceLocalPwd)`) | `RtpLatching.ReAimsWhenWeOfferedIceAndThePeerDeclinedIt`, et lui seul — les 10 autres passent, ce qui est le propre du bug : il ne se voit que sur la jambe où ICE est offert et décliné |
