...
- **The IP announced in the SDP has exactly one source**: `RTPSession::SetAnnouncedIp`/`GetAnnouncedIp` (`--public-ip`, else the first non-loopback IPv4 of the host name, resolved once at startup). Both control APIs read it — `Endpoint::GetMediaCandidates` (JSR-309) and `StartReceiving` (MCU, `returnVal[1]`) — so they cannot disagree, and a controller never has to carry the address in its own config. Do **not** re-derive a local address anywhere else: that duplication is what made every NAT deployment a per-caller guess. `main()` refuses to start when no address can be determined (see `readme.md`, *Adresse média annoncée*), which is what lets callers treat the value as always present.
