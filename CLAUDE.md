...
Le build est porté sur **AlmaLinux 9 / GCC 11** (chantier mené sur la branche `feat/alma_linux9` — voir `almalinux9_port_plan.md` pour l'état). La plupart des dépendances sont désormais **liées dynamiquement aux paquets système** (ffmpeg 5, OpenSSL 3, libsrtp2, x264, Magick++ 7, webrtc-audio-processing, usrsctp). Une seule bibliothèque est encore bâtie depuis les sources dans `./staticdeps` (mp4v2) ; xmlrpc-c est le paquet système dynamique (dépôt CRB), speex n'est plus bâti (le codec Speex passe par ffmpeg via libmedikit) et g722_1 non plus — le codec G.722.1/SIREN a été **retiré** du mediaserver et de libmedikit, il n'est donc ni bâti, ni lié, ni exposé par aucune API (un `libg722_1.la` peut traîner dans `staticdeps/lib` : c'est le résidu d'un build d'avant le retrait). L'essentiel du code codec/média vient maintenant du **sous-module `libmedikit`** (`third_party/fontventa/libmedikit`), qui porte lui-même le portage ffmpeg 5.
...
- **Une capacité n'est pas un usage — l'accélération matérielle en est le cas d'école.** `hardware.vaapi` de `/status/general` dit qu'un device VAAPI existe ; il ne dit pas qu'un encodeur s'en sert, parce que le repli matériel → logiciel est **silencieux par conception** (`FfVideoEncoder::FallbackToSoftware`). Ce sont les compteurs `VideoAccel` (`third_party/fontventa/libmedikit/video.cpp`, alimentés par les codecs eux-mêmes, publiés dans `hardware`) qui répondent : encodeurs et décodeurs ouverts, combien sur GPU, et les replis cumulés. `hwFallbacks` ne compte QUE les replis subis alors qu'un device était utilisable — sans GPU, le logiciel est la normale, pas un incident. Même règle que pour les codecs : **ne pas tenir une seconde comptabilité** côté contrôleur. **Le GPU est éteint par défaut** : `main()` appelle `Pict::DisableVAAPI` — avant toute ouverture de codec, un device déjà distribué survivant chez son porteur — sauf si `--use-hwaccel` est passée. Sans l'option, le device n'est même pas créé et `hardware.vaapi` vaut `false` sur une machine équipée. C'est assumé : un driver VAAPI défaillant casse le service (constaté sur Iris Xe + iHD + libavcodec 62, où `h264_vaapi` abort le processus), et l'accélération doit donc être un choix explicite de l'exploitant, vérifié ensuite sur `/status/general`. Détail : `docs/reference/status-http.md` §3.7-3.8.
- **Ne jamais vider (`avcodec_send_frame(ctx,NULL)`) un encodeur qui n'a reçu aucune trame** : `h264_vaapi` segfaute alors dans libavcodec, et le cas est banal — un participant dont l'encodeur s'ouvre à la négociation et qui raccroche avant la première image. `FfVideoEncoder::DrainCodec` s'en garde par le drapeau `fed`, remis à faux à chaque nouveau contexte.
