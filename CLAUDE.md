...
make -f mcu/Makefile.rpm mcu      # sortie : bin/debug/mcu
...
- Le `Makefile` et le `config.mk` de la racine sont **périmés/inutilisés** (ils référencent un répertoire `media/` inexistant). Le vrai build est `mcu/Makefile.rpm`, piloté par `install.ksh`.
- **Deux sous-modules in-tree sont obligatoires** : `mcu/Makefile.rpm` lie `$(MEDKITDIR)/libmedkit.a` (codecs, `-I$(MEDKITDIR)` AVANT `-Iinclude/` ; les `.o` de codecs qu'il fournit ont été retirés d'`OBJS`) et `$(BFCPDIR)/lib/libbfcp{dbg,rel}.a` (bibliothèque C BFCP, `-I$(BFCPDIR)/include`), tous deux par chemin complet. Plus aucune dépendance à `/opt/ives`.
- Principaux commutateurs de build dans `mcu/Makefile.rpm` : `DEBUG=yes` (défaut — `-g -O0`, compile dans `media/build/debug`, lie `libbfcpdbg.a`), `LOG=yes` (`-DLOG_`), `MOTELI=yes` (active le backend moteli RabbitMQ/protobuf — encore bâti depuis les sources, pas encore porté sur el9), `VADWEBRTC=yes` (VAD via l'APM système `webrtc-audio-processing`, `pkg-config`). Le standard C++ est `gnu++17` (avec `-Werror=return-type`).
- `DISTRO` dans `Makefile.rpm` ne détecte que `fc`/`el5`/`el6` ; la **branche par défaut (`else`) est le mode lien dynamique AlmaLinux 9** (il n'y a pas encore d'`el9` explicite).
...
- **Serveur XML-RPC** (`xmlrpcserver`, `xmlhandler`) — l'API de contrôle principale. Les tables de commandes (`mcuCmdList`, `broadcasterCmdList`, `mediagatewayCmdList`, `jsr309CmdList`) associent les noms de méthodes RPC à des handlers dans `xmlrpcmcu.cpp`, `xmlrpcbroadcaster.cpp`, etc.
...
- `mediagateway`/`mediabridgesession` font le pont entre transports (cible `mediagw`).
...
Les en-têtes sont dans `mcu/include/`, les sources dans `mcu/src/`. La liste d'objets et les options de lien de `mcu/Makefile.rpm` font foi sur ce qui est réellement compilé.
...
- **L'adresse IP annoncée dans le SDP a exactement une source** : `RTPSession::SetAnnouncedIp`/`GetAnnouncedIp` (`--public-ip`, sinon la première IPv4 non loopback du nom d'hôte, résolue une fois au démarrage). Les deux API de contrôle la lisent — `Endpoint::GetMediaCandidates` (JSR-309) et `StartReceiving` (MCU, `returnVal[1]`) — donc elles ne peuvent pas se contredire, et un contrôleur n'a jamais à porter l'adresse dans sa propre configuration. Ne **pas** redériver une adresse locale ailleurs : c'est cette duplication qui faisait de chaque déploiement derrière NAT une devinette par appelant. `main()` refuse de démarrer si aucune adresse ne peut être déterminée (voir `readme.md`, *Adresse média annoncée*), et c'est ce qui permet aux appelants de traiter la valeur comme toujours présente.
