...
- Principaux commutateurs de build dans `mcu/Makefile` : `DEBUG=yes` (défaut — `-g -O0`, compile dans `media/build/debug`, lie `libbfcpdbg.a`), `LOG=yes` (`-DLOG_`), `MOTELI=yes` (active le backend moteli RabbitMQ/protobuf — encore bâti depuis les sources, pas encore porté sur el9). La VAD n'a plus de commutateur : elle est toujours bâtie, l'archive venant du sous-module `libvad`. Le standard C++ est `gnu++17` (avec `-Werror=return-type`).
...
- Le BFCP (contrôle de parole pour le partage de document/écran) en service vient du **sous-module `third_party/libbfcp`**, bâti in-tree (en-têtes `-I$(BFCPDIR)/include`, archive `libbfcp{dbg,rel}.a` liée par chemin complet — plus de `/opt/ives`) ; son seul appelant est `src/shareddocmixer.cpp`. Le dépôt porte **une seconde pile BFCP, en C++, qui n'est dans aucune cible du Makefile** : `src/bfcp/` et `include/bfcp/`. La modifier ne change pas le binaire, et rien ne le signale — un chantier transversal qui balaie le dépôt par `grep` la touchera pour rien. Son transport est une connexion WebSocket, pas TCP : ce n'est pas un substitut direct. État des deux piles et ce que demanderait la réinternalisation : `docs/reference/bfcp.md`.
