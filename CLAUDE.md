...
Le build est porté sur **AlmaLinux 9 / GCC 11**, et sur **ffmpeg 9** (libavutil 61) depuis le 2026-09-22 : la migration est faite, mediaserver et libmedikit compris. Le code ne porte **aucune garde de version** ffmpeg (`#if LIBAVUTIL_VERSION_MAJOR`) et n'en veut pas : une seule version est visée à la fois. La plupart des dépendances sont **liées dynamiquement aux paquets système** (ffmpeg 9, OpenSSL 3, libsrtp2, x264, Magick++ 7, usrsctp). Une seule bibliothèque est encore bâtie depuis les sources dans `./staticdeps` (mp4v2) ; xmlrpc-c est le paquet système dynamique (dépôt CRB), speex n'est plus bâti (le codec Speex passe par ffmpeg via libmedikit) et g722_1 non plus — le codec G.722.1/SIREN a été **retiré** du mediaserver et de libmedikit, il n'est donc ni bâti, ni lié, ni exposé par aucune API (un `libg722_1.la` peut traîner dans `staticdeps/lib` : c'est le résidu d'un build d'avant le retrait). L'essentiel du code codec/média vient maintenant du **sous-module `libmedikit`** (`third_party/fontventa/libmedikit`), qui suit la même version de ffmpeg. **Piège de mise à jour** : ni ce dépôt ni le sous-module ne suivent les en-têtes système dans leurs `.d`. Après un changement de paquet `ffmpeg-devel`, nettoyer AVANT de reconstruire (`./install.ksh clean`, ou les `make clean` des deux arbres) — sinon les objets mélangent deux ABI, et ça ne se voit qu'au lien ou à l'exécution.

```sh
# 1. Installer les dépendances de build système :
./install.ksh prereq       # dnf/yum ou apt selon la distribution detectee (voir README)
...
#    libvad), les bâtit in-tree, bâtit les dépendances source dans
...
- **libmedkit est prise par `pkg-config`**, et ses `.o` de codecs ont été retirés d'`OBJS`. Plus aucune dépendance à `/opt/ives`. Le BFCP n'est plus un sous-module : il est bâti ici, dans `$(BFCPSTACKOBJ)`.
- **ffmpeg n'est écrit en dur nulle part.** `third_party/fontventa/libmedikit/Makefile` demande ses flags à `pkg-config` (`libavcodec libswscale libavformat libavutil libswresample`) et **génère `libmedkit.pc`** — chemins absolus vers le clone, motif « uninstalled » des autotools, non versionné, régénéré à chaque build. `mcu/Makefile` le consomme via `PKG_CONFIG_PATH=$(abspath $(MEDKITDIR))` et y ajoute les deux modules qu'il utilise en propre : `libavfilter` et `libavdevice`. **`libpostproc` a été retiré** : aucune source du dépôt ne l'inclut, et Debian ne le paquetise pas — ne pas le réintroduire. ffmpeg y est déclaré en `Requires:` (et non `Requires.private:`) parce que la dépendance est publique — `ffvideocodec.h` inclut `<libavcodec/avcodec.h>` —, donc ses `-I` et ses `-l` remontent seuls au consommateur. Conséquence pratique : **bâtir contre un autre ffmpeg ne demande qu'un `PKG_CONFIG_PATH`**. Les deux Makefiles ont un **repli bruyant** sur les chemins historiques si pkg-config ne résout pas : `libmedkit.pc` n'existe qu'une fois le sous-module bâti, ce que `install.ksh` garantit mais pas un `make` lancé à la main sur un arbre neuf. Ce repli pointe sur `/usr/include/ffmpeg`, qui n'existe plus : un build qui l'emprunte a déjà un vrai problème en amont, et le warning le dit.
...
- Enregistrement/lecture MP4 : `mp4recorder`/`mp4player`/`mp4streamer` sont de fines coquilles au-dessus des **`mp4writer`/`mp4reader`** de libmedikit (libmp4v2 en dessous). L'enregistrement et la diffusion FLV/RTMP (`FLVEncoder`, `flvrecorder`, `rtmpflvstream`) restent dans `mcu`.
  **Exception, le Recorder JSR-309** (`src/jsr309/Recorder.cpp`) : il ne passe plus par `MP4Recorder` mais écrit par le **`FfMediaFileWriter`** de libmedikit (libavformat), donc son **conteneur suit l'extension** du nom de fichier et il n'y a plus de hint track. Conséquence normative, qui ne se devine pas : libavformat fige l'en-tête à la première trame écrite, donc **toutes les pistes se déclarent avant** — les `RecorderAttachTo…` doivent précéder `RecorderRecord`, et l'audio comme la vidéo sont *annoncés* (`FfMediaFileWriter::ExpectTrack`, 2 s) puisque leur codec n'arrive qu'avec le premier paquet RTP. Le transcodage audio n'est plus décidé par codec mais par conteneur (`IsCodecSupported`) : PCMU passe tel quel en `.mkv`, devient de l'AAC en `.mp4`. Contrat exploitant : `docs/JSR-309-API.md` §6.4.
- La VAD vient du **sous-module `third_party/libvad`** (`aks-tel/libvad`), dont on ne bâtit QUE les 12 sources libfvad : `compile_libvad` surcharge `LIB_SRC='$(FVAD_SRC)'` en ligne de commande, sans écrire un fichier dans le sous-module (IVèS ne le forke pas). Sa couche `sivr-vad` est délibérément écartée — elle ne compile pas hors FreeSWITCH, et sa machine à états ne rend pas le 0/1 par trame qu'attend `PipeAudioOutput`. Ne pas la réintroduire. Le dépôt amont ne porte ni `LICENSE` ni `PATENTS` : la notice BSD est reprise dans `LICENSE.libfvad` à la racine, et installée par les deux paquets. Référence : `docs/reference/vad.md`.
- Le BFCP (contrôle de parole pour le partage de document/écran) est **interne** : `mcu/src/bfcp/` et `mcu/include/bfcp/`, dans `$(BFCPSTACKOBJ)`. Plus aucun sous-module BFCP. Il couvre le codec binaire RFC 4582/8855, le contrôle de parole et deux transports, TCP et UDP, battus par le réacteur `RtpSessionSet` — pas de thread propre. **MCU seule, pas de TLS** (décision : `docs/architecture/adr-002-bfcp-pile-interne.md`). Son seul appelant côté conférence est `src/shareddocmixer.cpp`. Trois pièges qui ne se devinent pas — le champ Length d'un attribut exclut son bourrage, le bit M appartient à la position et non à l'attribut, et la version émise sur UDP est celle du pair (les endpoints en service parlent la version 1) : `docs/reference/bfcp.md`.
...
Deux sources ne sont dans aucune cible et sont **gardées délibérément** : `src/rtmpdebug.cpp` et `src/flvdump.cpp`, chacune un outil de diagnostic autonome avec son propre `main()`. Ne pas les supprimer comme du code mort.
