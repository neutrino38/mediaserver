...
| retard initial | **5 000 ms** (était 500 + 60 000) | 5 s d'initialisation (`kInitializationTime`, `aimd_rate_control.cc:155`) | corrigé le 2026-08-17, aligné sur le témoin. Les 60 s — un « TMMBR skipping delay » — rendaient **tout appel de moins de 61,5 s de vidéo inobservable** : la réestimation périodique est gardée par `lastChange + 1000 < now`, donc aucune trace `BWE: estimation` n'existait avant. Constaté en tentant la séance du lot 3 : deux appels de 50 s, zéro estimation. L'émission étant désormais verrouillée par la négociation et amortie par le `RembThrottler` (lot 2), l'estimateur n'a plus à se taire une minute. Test : `LEstimationArriveDansLesPremieresSecondes`. |
...
| montée multiplicative | `RateIncreaseFactor(…)`, **×1,048/s mesuré** | ×1,08/s **en découverte SEULEMENT** ; en régime, montée **additive** ≈ +14 kb/s·s à 1 Mb/s, soit ×1,014 | corrigé le 2026-08-18 contre les sources (`aimd_rate_control.cc:250-266`, `:342-356`, `GetNearMaxIncreaseRateBpsPerSecond`). Le ×1,08 n'est employé que **sans** estimation de capacité de lien ; dès qu'une surutilisation a eu lieu, le témoin passe en additive (plancher 4 000 bit/s·s). **Nous montons donc plus vite que lui en régime**, et la lenteur de re-montée mesurée ne vient pas de ce facteur. La divergence est ailleurs : le témoin **efface** sa mémoire de capacité quand le débit mesuré s'en écarte, ce qui lui rend sa rampe rapide ; la nôtre se nourrit de nos propres consignes (`UpdateMaxBitRateEstimate(fmax(currentBitRate, incomingBitRate))`) et ne s'efface jamais. Détail : `docs/reference/kalman-vs-webrtc.md` §É5. |
